...
Real-time fraud alert monitoring with auto-refresh capability and live alert queue.
"""
...
def render_alert_queue(alerts):
    ...
    st.markdown(f"### 🚨 Active Alert Queue ({len(alerts)} Pending)")
...
def render_top_rules(rules):
    ...
    fig = px.bar(
        ...
        title='Top 10 Most Triggered Detection Rules'
    )
...
def render_scenario_breakdown(scenarios):
    ...
    fig = px.pie(
        ...
        values='Count',
        names='Scenario',
        title='Fraud Activity Distribution by Scenario'
    )
...
def render():
    """Render the Transaction Monitoring System page"""
...
def render_dashboard_content(time_window):
    ...
    try:
        ...
        with col2:
            ...
            # Scenario breakdown
            st.markdown("### 🎯 Fraud Scenario Activity")
